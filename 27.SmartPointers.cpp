...
// if we have to create data in heap memory we have to use new and delete keyword.
...
// So basically, if one shared pointer copied to other shared pointer it increase the ref count.
...
int main() 
{
	// unique pointer
	{
		...
		std::unique_ptr<Entity> entity = std::make_unique<Entity>(); // But this is a prefered way for throw exception
	...
	// Shared Pointer
	{
		std::shared_ptr<Entity> e;
		{
			...
			e = e2; // copying one shared pointer to other shared pointer hence increasing the ref count to 2 for e2 address
			// and also Entity destroyed will print for shared pointer e because e now have e2 address and e default address is deleted.
		}
	...
	//here Entity destroyed will print for shared pointer e2.
